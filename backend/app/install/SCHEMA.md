...
- **courses / course_lessons**：课程卡片与课节内容，含标题、副标题、标签、配图以及中英文例句，支持批量读取展示。
- **admin_orders / admin_order_items**：管理端预置订单及明细，使用字符串型订单号存储，便于直接展示静态示例。
- **admin_user_profiles**：管理端示例会员画像，记录手机号、等级、消费与充值记录等。
